...
class GameInstaller {
	...
	public function setGame( $game )
	{
		...
		if ($this->gameExists( $game )) {
			...
			if (file_exists( $manifest )) {
				...
				if(isset($arr['game'][0]['value']['actions'][0]['value']['configs'][0]['value']))
	...
	/**
	 * Get the config file for server
	 *
	 * @param int $config
	 * @return bool/string
	 * @access public
	...
	{
		/*if(!isset($this->sftpConnection) || !is_object($this->sftpConnection) || $this->sftpConnection == NULL)
			return false;*/
		if (empty( $this->gameServerPath ))
			return false;
		if(!isset($this->actions['configs']['file'][$config]['value']) || empty($this->actions['configs']['file'][$config]['value']))
			return false;
		if (trim( $this->sshConnection->exec('test -f '.$this->gameServerPath.$this->actions['configs']['file'][$config]['value']." && echo 'true' || echo 'false'") ) == 'false')
			return false;
		return $this->sshConnection->get($this->gameServerPath.$this->actions['configs']['file'][$config]['value']);
	}
}
?>